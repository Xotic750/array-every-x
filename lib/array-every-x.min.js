 ...
 */
"use strict";var isWorking,cachedCtrs=_dereq_("cached-constructors-x"),ArrayCtr=cachedCtrs.Array,castObject=cachedCtrs.Object,nativEvery="function"==typeof ArrayCtr.prototype.every&&ArrayCtr.prototype.every;if(nativEvery){var attempt=_dereq_("attempt-x"),spy=0,res=attempt.call([1,2],nativEvery,function(item){return spy+=item,!0});if((isWorking=!1===res.threw&&!0===res.value&&3===spy)&&(spy="",isWorking=!1===(res=attempt.call(castObject("abc"),nativEvery,function(item,index){return spy+=item,2!==index})).threw&&!1===res.value&&"abc"===spy),isWorking&&(spy=0,isWorking=!1===(res=attempt.call(function(){return arguments}(1,2,3),nativEvery,function(item,index){return spy+=item,1!==index})).threw&&!1===res.value&&3===spy),isWorking&&(spy=0,isWorking=!1===(res=attempt.call({0:1,1:2,3:3,4:4,length:4},nativEvery,function(item){return spy+=item,!0})).threw&&!0===res.value&&6===spy),isWorking){var doc="undefined"!=typeof document&&document;if(doc){spy=null;var fragment=doc.createDocumentFragment(),div=doc.createElement("div");fragment.appendChild(div),isWorking=!1===(res=attempt.call(fragment.childNodes,nativEvery,function(item){spy=item})).threw&&!1===res.value&&spy===div}}if(isWorking){(function(){return!1===Boolean(this)})()&&(spy=null,isWorking=!1===(res=attempt.call([1],nativEvery,function(){spy="string"==typeof this},"x")).threw&&!1===res.value&&!0===spy)}if(isWorking){spy={};var fn=['return nativEvery.call("foo", function (_, __, context) {','if (Boolean(context) === false || typeof context !== "object") {',"spy.value = true;}});"].join("");isWorking=!1===(res=attempt(Function("nativEvery","spy",fn),nativEvery,spy)).threw&&!1===res.value&&!0!==spy.value}}var $every;if(nativEvery)$every=function every(array,callBack){var args=[callBack];return arguments.length>2&&(args[1]=arguments[2]),nativEvery.apply(array,args)};else{var splitIfBoxedBug=_dereq_("split-if-boxed-bug-x"),toLength=_dereq_("to-length-x").toLength2018,isUndefined=_dereq_("validate.io-undefined"),toObject=_dereq_("to-object-x"),assertIsFunction=_dereq_("assert-is-function-x");$every=function every(array,callBack){var object=toObject(array);assertIsFunction(callBack);var thisArg,iterable=splitIfBoxedBug(object),length=toLength(iterable.length);arguments.length>2&&(thisArg=arguments[2]);for(var noThis=isUndefined(thisArg),i=0;i<length;i+=1)if(i in iterable){var item=iterable[i];if(!1===(noThis?callBack(item,i,object):callBack.call(thisArg,item,i,object)))return!1}return!0}}module.exports=$every},{"assert-is-function-x":2,"attempt-x":3,"cached-constructors-x":4,"split-if-boxed-bug-x":27,"to-length-x":30,"to-object-x":32,"validate.io-undefined":40}],2:[function(_dereq_,module,exports){/**
